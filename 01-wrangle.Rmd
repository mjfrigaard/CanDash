...
This script outlines the steps to wrangle the Cannabis data. We used an open online retail data set for the week-over-week visualizations (`OnlineRetail`), and combined this with categories from the [`kushyapp`](https://github.com/kushyapp/cannabis-dataset).
...
```{r SmOnlineRetail}
SmOnlineRetail <- sample_frac(tbl = OnlineRetail, size = 1/10)
SmOnlineRetail %>% glimpse(78)
```
...
SmOnlineRetail <- SmOnlineRetail %>% 
    dplyr::mutate(
                ...
                dow = lubridate::day(invoice_date), # day of the week 
                week = lubridate::week(invoice_date), # week of the year
                yr = lubridate::year(invoice_date), # year 
...
### Create a 'cost-per-invoice' data set

This will group the data by the invoice number and calculate the cost per invoice (`cost_per_invoice`) by multiplying quantity and unit price. 

```{r CostPerInvoice}
CostPerInvoice <- SmOnlineRetail %>% 
  group_by(invoice_no) %>% 
  dplyr::mutate(cost_per_invoice = quantity*unit_price) %>% 
  dplyr::ungroup() %>% 
...
Here we're going to combine the distinct names for `product_name` with the `CostPerInvoice` data frame. 
...
Now I want to get a sample from the `CostPerInvoice` table that is the same size of the online retail data, but first make sure these are sorted by the `quantity`. I also want to add an id that matches the id's in the `TidyProductsBrands`.
...
```{r CannabisWowData}
CannabisWowData <- CostPerInvSamp %>% 
