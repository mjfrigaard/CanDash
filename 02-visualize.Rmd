...
These data came from the [kushy cannabis data set](https://github.com/kushyapp/cannabis-dataset). 

```{r import-CannabisData, message=FALSE, warning=FALSE}
...
This `data.frame` is a combination of simulated sales data and some product and brand data. 
...
```{r hide_legend-toWebGL-ggplotly, message=FALSE, warning=FALSE}
plotly::hide_legend(plotly::toWebGL(
      plotly::ggplotly(p = gg_top10_product_categories)))
```

This tells me `Concentrate` are the most common brand category. I can also see `Edibles` and `Vapes` are numbers two and three. 

## Top 10 Cannabis Brands

These are the most common product categories, sorted with the most common first. 

```{r visualize-Top10Brands}
CanData %>% 
...
```{r gg_top10_brands-plotly, warning=FALSE, message=FALSE}
plotly::hide_legend(plotly::toWebGL(
...
Now we can group these two variables and count them both, limiting this to the top 25.

```{r top25-brand-categories}
Top25Data <- CanData %>%
...
```{r geom-col-Top25Data, message=FALSE, warning=FALSE}
# define labels ----
...
This creates a grouped data.frame/tibble on `product_category` and `units_sold`.

```{r create-GroupedCanData}
...
Here we see there is a data.frame with 54 rows (one for each week). And this data set is still grouped, but that won't be an issue.
...
We will create a quarter drop-down variable for the dashboard. The data we created above allows us to plot the week over week sales by their brand categories. 
...
These are the week over week annual sales (quantity), faceted by `month`, which needs at least one level. 
...
```{r plotly-gg_wow_sales, message=FALSE, warning=FALSE}
# Convert this to a `plotly` object.
plotly::hide_legend(
...
```{r ggWowQuantity, message=FALSE, warning=FALSE}
...
```{r plotly-ggTop100MonthlyLocationSales, message=FALSE, warning=FALSE}
# convert to plotly
plotly::hide_legend(
  plotly::toWebGL(
...
Faceting by region and coloring by region while having months on the x axis is helpful for spotting trends. Graphs like this can help us spot gaps in total sales by location (i.e. California vs. Manitoba).
