...
```{r week-over-week-by-brand-categories}
# these are the labels
labs_wow_brand_cat_annual_sales <- ggplot2::labs(
  y = 'Sales', 
  x = 'Week', 
  title = 'Week Over Week Sales By Brand Category',
  subtitle = "Simulated Cannabis Sales Data",
  caption = "graphic and analysis by PDG") 

WowCostPerInvoice %>% 
  # filter to Q3
  dplyr::filter(month %in% c("Apr", "May", "Jun") & 
                  # only concentrates and flowers
           brand_category %in% c("Concentrates", "Flower", "Medical")) %>% 
  # remove missing 
  tidyr::drop_na() %>% 
  # this will put week_year on the x
    ggplot2::ggplot(aes(x = week_year, 
                        # and the week over week quantity on the y
                        y = wow_brand_quantity,
                        group = brand_category)) +
  # add the line plot
    ggplot2::geom_line() +
  # and the point
  ...
  # this adds the percent on the y axis
    ggplot2::scale_y_continuous(labels = scales::percent_format(accuracy = 1),) +
  # here we facet by the month
    ggplot2::facet_wrap(brand_category ~ month, 
                        # 
                        scales = "free") + 
    ggplot2::theme(axis.text.x = 
                     element_text(angle = 45, 
                                  hjust = 0.5, 
                                  vjust = 0.5)) +
  labs_wow_brand_cat_annual_sales
```

```{r}
BrandCatWeekOverWeek %>% 
  dplyr::filter(month %in% c("May") & 
           brand_category %in% c("Concentrates"))
```
