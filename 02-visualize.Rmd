...
CannabisData <- readr::read_csv("data/processed/2020-03-19-CannabisWowData.csv")
```
...
CanData %>% 
  ...
  ggplot2::ggplot(aes(x = brand_category, y = n)) +
    ...
    ggplot2::labs(x = "Brand Category",
                  y = "Count",
                  title = "Top 5 Cannabis Brand Categories") +
    ggplot2::coord_flip()
```
...
Now we can group these two variables and count them both, limiting this to the top 50. 
...
Top25Data <- CanData %>%
  ...
  dplyr::filter(brand_category %in% c("Concentrates", "Edibles", 
                                      "Flower", "Medical")) %>% 
  utils::head(25) %>% 
  dplyr::mutate(brand = reorder(x = brand, X = brand_cat_count))
...
readr::write_rds(x = Top25Data, 
          path = paste0("data/processed/", 
                        ...
                        "-Top25Data.rds"))
# export Top25Data -----
write_csv(as.data.frame(Top25Data), 
          path = paste0("data/processed/", 
                        base::noquote(lubridate::today()),
                        "-Top25Data.csv"))
```

```{r geom-col-Top25Data}
# define labels ----
...
This creates a grouped data.frame/tibble on `brand_category` and `quantity`.

```{r create-Top100BrandCatsData}
...
Create quarter drop-down variable for the dashboard. 
...
MonthlyLocationSales <- CannabisData %>% 
    ...
    dplyr::mutate(sales_region = case_when(
              ...
              location == "Michigan" ~ "Northeast/East North Central",
              
              location == "Salisbury Postcode Area (UK)" ~ "South West England",

              location == "California" ~ "West/Pacific",
              location == "Oregon" ~ "West/Pacific",
              location == "Washington" ~ "West/Pacific",
              
              location == "British Columbia" ~ "Canadian",
              location == "Alberta" ~ "Canadian",
              location == "Manitoba" ~ "Canadian",
              location == "Saskatchewan" ~ "Canadian",
              location == "Nova Scotia" ~ "Canadian",
              location == "Ontario" ~ "Canadian",
              location == "Newfoundland" ~ "Canadian")) %>% 
    dplyr::group_by(floor_month, location) %>% 
    # remove the SP location
    dplyr::filter(sales_region != "South West England")
# check new data 
...
ggTop100MonthlyLocationSales <- MonthlyLocationSales %>%
    ...
    ggplot2::facet_wrap(. ~ sales_region, 
                        scales = "free") + 
...
# convert to plotly
plotly::toWebGL(plotly::ggplotly(ggTop100MonthlyLocationSales))
```
