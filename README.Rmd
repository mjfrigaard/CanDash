...
We had to simulate the cost data for the products in the `Products` and `Brands` tables. Read more about this process in the [`01.1-wrangle.md` file](). 

```{r data-objects, echo=FALSE}
fs::dir_tree(".", recurse = TRUE)
```

## Import

The data for the dashboard are imported below (We pick the latest version).

```{r import-CannabisWowData, message=FALSE, warning=FALSE}
# fs::dir_tree("data/processed/")
CannabisWowData <- readr::read_csv("data/processed/2020-04-17-CannabisWowData.csv")
CannabisWowData %>% dplyr::glimpse(78)
```

## Date ranges 

Below we summarize the date ranges in this dataset.
...
We can see the overwhelming number of the data are `Topicals` and `Concentrates` from `California`, in the months of `Nov/Oct`, etc.
